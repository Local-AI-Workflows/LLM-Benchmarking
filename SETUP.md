...
- Python 3.8+
- Node.js 18+ and npm 8+ (check with `node --version`)
- MongoDB (running locally or remotely)

> **Note**: If you have Node.js < 18, see `frontend/UPGRADE_NODE.md` for upgrade instructions.
...
## Step 2: Set Up MongoDB with Docker Compose

The easiest way to set up MongoDB is using Docker Compose:

```bash
# Start MongoDB (and optionally the API server)
docker-compose up -d
...
**Important**: Make sure you have Node.js 18+ installed. Check with:
```bash
node --version
```

If you have an older version, see `frontend/UPGRADE_NODE.md` for upgrade instructions.
